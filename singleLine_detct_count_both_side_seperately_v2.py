...
while True:
    ...
    if ret:
        ...
        # Detector and tracking for Zone 1
        detector.setInput(blob)
        person_detections_zone1 = detector.forward()
        rects_zone1 = []
        
        for i in np.arange(0, person_detections_zone1.shape[2]):
            confidence = person_detections_zone1[0, 0, i, 2]
            if confidence > 0.4:
                idx = int(person_detections_zone1[0, 0, i, 1])
        
                if CLASSES[idx] != "person":
                    continue
        
                person_box = person_detections_zone1[0, 0, i, 3:7] * np.array([W, H, W, H])
        ...
        # Detector and tracking for Zone 2
        detector.setInput(blob)
        person_detections_zone2 = detector.forward()
        rects_zone2 = []
        
        for i in np.arange(0, person_detections_zone2.shape[2]):
            confidence = person_detections_zone2[0, 0, i, 2]
            if confidence > 0.7:
                idx = int(person_detections_zone2[0, 0, i, 1])
        
                if CLASSES[idx] != "person":
                    continue
        
                person_box = person_detections_zone2[0, 0, i, 3:7] * np.array([W, H, W, H])
                (startX, startY, endX, endY) = person_box.astype("int")
                # Check which side of the line the person is on
                if not startX + endX < line_coordinates[0][0] + line_coordinates[1][0]:
