...
tracker2 = CentroidTracker(maxDisappeared=250, maxDistance=100)
...
out_ = cv2.VideoWriter("output_multiple_roi_people_count.mp4", fourcc_codec, fps, capture_size)

# Function to draw rectangle using four points
...
while True:
    ...
    if ret:
        ...
        #frame2_pts = np.array([[220,532], [532, 10], [920,30], [774, 584]], np.int32)
        #frame2_pts = np.array([[295,403], [431, 3], [1048,4], [650, 580]], np.int32)
        #frame2_pts = np.array([[295,403], [367, 0], [1060,0], [900, 580]], np.int32)
        frame2_pts = np.array([[280,480], [308, 6], [855,8], [840, 585]], np.int32)
        #frame2_pts = np.array([[300,405], [335, 10], [1070,5], [423, 582]], np.int32)
        frame2 = img_f[frame2_pts[1][1]:frame2_pts[3][1], frame2_pts[0][0]:frame2_pts[2][0]]
        ...
        #frame2 =draw_rectangle(img_f.copy(), frame2_pts)
        #frame2 =img_f[152:584, 10:920]
        draw_rectangle(img_f, frame2_pts)
        #frame2 = img_f[frame2_pts[0][1]:frame2_pts[2][1], frame2_pts[0][0]:frame2_pts[2][0]]
        ...
        for (objectId, bbox) in objects1.items():
            ...
            #text = "Z1 id:{}, ft:{}sec".format(objectId, int(dwell_time[objectId]))
            #text = "Zone-1 Id:{}".format(objectId, int(dwell_time[objectId]))
            text = "Zone-1".format(objectId, int(dwell_time[objectId]))
        ...
        for (objectId2, bbox2) in objects2.items():
            ...
            #text2 = "Z2 id:{}".format(objectId2)
            text2 = "Zone-2".format(objectId2)
            cv2.putText(frame2, text2, (x1, y1 - 5), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 1)
        ...
        cv2.putText(img_f, opc_txt1, (850, 110), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (50, 255, 50), 1)
        
        # Count current person count and total person count for Zone-2
        opc_count2 = len(objectId_list2)
        opc_txt2 = "Zone-2: {}".format(opc_count2)
        cv2.putText(img_f, opc_txt2, (850, 140), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (0, 255, 255), 1)
        if opc_count2==2:
            break
